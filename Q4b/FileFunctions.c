...
unsigned int binReadShortInt(short int* num, unsigned int size, FILE *filePtr)
{
    if (fread(num, sizeof(short int), size, filePtr) != size)
    {
        printf("Error reading short int from file\n");
        exit(READ_FILE_ERROR);
    }
    return size;
}
...
unsigned int binWriteUnsignedLong(unsigned long num, FILE *filePtr)
{
    int size = 1;
    if (fwrite(&num, sizeof(unsigned long), size, filePtr) != size)
    {
        printf("Error writing unsigned long to file\n");
        exit(WRITE_FILE_ERROR);
    }
    return size;
}
